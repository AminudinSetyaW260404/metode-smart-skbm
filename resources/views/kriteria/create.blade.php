<x-app-layout>
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-slate-800">{{ isset($kriteria) ? 'Edit' : 'Tambah' }} Kriteria</h1>
        <p class="text-slate-500 mt-1">{{ isset($kriteria) ? 'Ubah data kriteria' : 'Tambahkan kriteria penilaian baru' }}</p>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 max-w-2xl">
        <form action="{{ isset($kriteria) ? route('kriteria.update', $kriteria) : route('kriteria.store') }}" method="POST">
            @csrf
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $kriteria->nama ?? '') }}" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition" placeholder="Contoh: IPK, Penghasilan, Semester" required>
            ...
            <div class="flex gap-3 mt-8">
                <button type="submit" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl shadow-sm transition">
