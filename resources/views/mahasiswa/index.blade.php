<x-app-layout>
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-slate-500 mt-1">Kelola data mahasiswa penerima beasiswa</p>
        </div>
        <a href="{{ route('mahasiswa.create') }}" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl shadow-sm transition flex items-center gap-2">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-slate-50">
                        ...
                        <th class="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Angkatan</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($mahasiswas as $i => $m)
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-6 py-4 text-sm font-medium text-slate-800">{{ $m->nama }}</td>
                            <td class="px-6 py-4 text-sm text-slate-500">{{ $m->jurusan ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm text-slate-500">{{ $m->angkatan ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm font-semibold text-slate-700">{{ $m->ipk }}</td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('mahasiswa.edit', $m) }}" class="p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <form action="{{ route('mahasiswa.destroy', $m) }}" method="POST" onsubmit="return confirm('Yakin hapus?')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <p class="text-slate-400">Belum ada data mahasiswa</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
